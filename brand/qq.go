...
func (receiver *QqEvent) GetType() string {
	return "qq"
}

// Explain explain 解析命令函数
// 将任务以此拿出来进行判断
func (e *QqEvent) Explain(bodyData []byte) {
	for i := 0; i < len(CoralBot.Tasks); i++ {
		task := CoralBot.Tasks[i]
...
// 过滤
func (e *QqEvent) filterStart(task CoralBot.Task) error {
...
//
//func (e *QqEvent) pluginFilterStart(task CoralBot.Task) error {
//	var pts []CoralBot.Task
...
// 类型断言
func (e *QqEvent) typeAsserts(key interface{}) (interface{}, error) {
	switch key.(type) {
	case *int64:
